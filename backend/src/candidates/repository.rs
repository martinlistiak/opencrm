...
pub async fn create(
    ...
    .bind(&skills)
    .bind(req.seniority.as_ref().filter(|s| !s.is_empty()))
    .bind(req.availability.as_ref().filter(|s| !s.is_empty()).map(|s| s.as_str()).unwrap_or("unknown"))
...
pub async fn update(
    ...
    .bind(&req.skills)
    .bind(req.seniority.as_ref().filter(|s| !s.is_empty()))
    .bind(req.availability.as_ref().filter(|s| !s.is_empty()))
    .bind(req.salary_expectation)
